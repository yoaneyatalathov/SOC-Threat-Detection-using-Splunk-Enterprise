...
The following architecture illustrates the end-to-end **Security Operations Center (SOC)** workflow implemented in this project. Simulated cyber attacks generate security events that are collected, analyzed, visualized, and handled through a structured Incident Response process.
...
3. **Log Forwarding**
   - Splunk Universal Forwarder forwards logs to Splunk Enterprise.

4. **Threat Detection**
   - Splunk Enterprise analyzes security events using custom **SPL detection rules**.

5. **Security Monitoring**
   - Dashboards provide real-time visibility into security events and attack activities.

6. **Alert Generation**
   - Detection rules automatically generate alerts when suspicious activities are identified.

7. **Incident Response**
   - SOC Analyst investigates alerts and follows the NIST Incident Response lifecycle:
     - Detection
     - Containment
     - Eradication
     - Recovery
...
The incident response process follows the **NIST SP 800-61 Computer Security Incident Handling Guide**. After an alert is generated by Splunk Enterprise, the SOC Analyst performs a structured response process to investigate and mitigate the incident.

![Incident Response Workflow](architecture/incident-response-workflow.png)

### Incident Response Phases

1. **Detection**
   - Security events are detected using Splunk Enterprise detection rules and automated alerts.

2. **Containment**
   - Identify the source IP, assess the scope of the incident, and recommend access restrictions or firewall/WAF rules.

3. **Eradication**
   - Review logs, validate application security, and recommend remediation such as input validation or configuration improvements.

4. **Recovery**
   - Verify that the monitored systems return to normal operation and continue security monitoring.

5. **Lessons Learned**
   - Document the incident, evaluate detection rules, and improve monitoring for future incidents.

Both attack simulations (Port Scan and SQL Injection) successfully generated alerts and were processed through this Incident Response workflow.
